...
import { Mail, Users, BarChart3, Target } from "lucide-react";

interface EmailMarketingFeaturesProps {
  features: string[];
  title?: string;
  className?: string;
}

export function EmailMarketingFeatures({ 
  features, 
  title = "What You Get",
  className = "" 
}: EmailMarketingFeaturesProps) {
  ...
  // Map features to appropriate email marketing related icons
  const getFeatureIcon = (feature: string, index: number) => {
    const iconMap = [Mail, Users, BarChart3, Target];
    const IconComponent = iconMap[index] || Mail;
