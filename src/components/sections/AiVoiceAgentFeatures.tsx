...
import { Mic, Database, GitBranch, FileText } from "lucide-react";

interface AiVoiceAgentFeaturesProps {
  features: string[];
  title?: string;
  className?: string;
}

export function AiVoiceAgentFeatures({ 
  features, 
  title = "What You Get",
  className = "" 
}: AiVoiceAgentFeaturesProps) {
  ...
  // Map features to appropriate icons for AI Voice Agent
  const getFeatureIcon = (feature: string, index: number) => {
    const iconMap = [Mic, Database, GitBranch, FileText]; // Natural voice, CRM integration, Call flow, Real-time transcription
