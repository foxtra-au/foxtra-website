...
import { FileText, Search, TrendingUp, BarChart3 } from "lucide-react";

interface BlogArticleFeaturesProps {
  features: string[];
  title?: string;
  className?: string;
}

export function BlogArticleFeatures({ 
  features, 
  title = "What You Get",
  className = "" 
}: BlogArticleFeaturesProps) {
  ...
  // Map features to appropriate blog/article writing related icons
  const getFeatureIcon = (feature: string, index: number) => {
    const iconMap = [FileText, Search, TrendingUp, BarChart3];
    const IconComponent = iconMap[index] || FileText;
