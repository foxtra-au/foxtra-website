...
interface GoogleAdsFeaturesProps {
  features: string[];
  title?: string;
  className?: string;
}

export function GoogleAdsFeatures({ 
  features, 
  title = "What You Get",
  className = "" 
}: GoogleAdsFeaturesProps) {
  ...
  // Map features to appropriate Google Ads related icons
  const getFeatureIcon = (feature: string, index: number) => {
    const iconMap = [Target, TrendingUp, BarChart3, Zap];
