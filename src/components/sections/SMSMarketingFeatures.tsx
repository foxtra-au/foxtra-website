...
interface SMSMarketingFeaturesProps {
  features: string[];
  title?: string;
  className?: string;
}

export function SMSMarketingFeatures({ 
  features, 
  title = "What You Get",
  className = "" 
}: SMSMarketingFeaturesProps) {
  ...
  // Map features to appropriate SMS marketing related icons
  const getFeatureIcon = (feature: string, index: number) => {
    const iconMap = [MessageSquare, Users, BarChart3, Target];
    const IconComponent = iconMap[index] || MessageSquare;
