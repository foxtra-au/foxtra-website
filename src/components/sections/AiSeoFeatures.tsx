...
interface AiSeoFeaturesProps {
  features: string[];
  title?: string;
  className?: string;
}

export function AiSeoFeatures({ 
  features, 
  title = "What You Get",
  className = "" 
}: AiSeoFeaturesProps) {
  ...
  // Map features to appropriate SEO-related icons
  const getFeatureIcon = (feature: string, index: number) => {
    const iconMap = [Search, TrendingUp, FileText, BarChart3];
    const IconComponent = iconMap[index] || Search;
