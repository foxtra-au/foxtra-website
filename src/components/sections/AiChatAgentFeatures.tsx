...
interface AiChatAgentFeaturesProps {
  features: string[];
  title?: string;
  className?: string;
}

export function AiChatAgentFeatures({ 
  features, 
  title = "What You Get",
  className = "" 
}: AiChatAgentFeaturesProps) {
  ...
  // Map features to appropriate icons for AI Chat Agent
  const getFeatureIcon = (feature: string, index: number) => {
    const iconMap = [Clock, Zap, Target, Calendar]; // 24/7, Multi-platform, Lead qualification, Appointment booking
